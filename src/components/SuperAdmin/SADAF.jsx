...
const SADAF = () => {
  ...
  const stats = [
    { icon: "👴", label: "Senior Citizen", count: senior, textColor: "text-blue-600" },
    ...
    { icon: "👤", label: "Indigenous People", count: ip, textColor: "text-blue-600" },
  ];
  ...
  const fetch_masterlist = async () => {
    try {
      ...
      const student = data.filter(item => item.occupation === 'Student').length;
      setStudent(student);
  ...
  const filteredData = evacuees.filter((user) => {
    ...
    const centerMatches = selectedBarangay
      ? user.barangay && user.barangay.toLowerCase().includes(selectedBarangay.toLowerCase())
      : true;
          ...
          <div className="p-6 max-w-7xl mx-auto">
            ...
            <div className="flex justify-between mt-10">
              <div className="flex flex-wrap gap-4 mb-6 justify-end">
                <select value={selectedBarangay} onChange={(e) => setSelectedBarangay(e.target.value)} className="border rounded-md px-3 py-2">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={handleExportPDF} className="btn btn-success text-white font-bold">
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Evacuation Center</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredData.map((evacuee) => (
                      <tr key={evacuee.id}>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{evacuee.id}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{evacuee.name}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{evacuee.evacuation_center}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{evacuee.date}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
